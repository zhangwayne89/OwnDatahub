CREATE OR REPLACE VIEW VW_RPT_STRUCTURING_RISK AS
WITH tmp_bond_structured AS
 (SELECT *
    FROM bond_position_structured
   WHERE isdel = 0
  --AND end_dt = ${v_date}
  ),
tmp_bond_pool AS
 (SELECT bp1.item_id AS secinner_id, bp2.pool_nm, bp2.group_type
    FROM bond_pool_item bp1, bond_pool bp2
   WHERE bp1.bond_pool_id = bp2.bond_pool_id
     AND bp2.group_type IN (3, 4)
     AND bp2.isdel = 0
     AND bp1.isdel = 0),
tmp_bond_party AS
...
SELECT basinfo.security_cd, --  债券代码
       ...
       CASE
         WHEN tmp_bond_pool.group_type = 3 THEN
          REPLACE(tmp_bond_pool.pool_nm, '预警池')
         ELSE
          NULL
       END AS bond_warn_level, --债券预警级别
       CASE
         WHEN tmp_bond_pool.group_type = 4 THEN
          'Y'
         ELSE
          'N'
       ...
       tmp_bond_structured.amt_cost, --面值（亿元）
       tmp_bond_structured.amt_marketvalue, --市值（亿元）
       tmp_bond_structured.position_num, --数量
       tmp_bond_structured.currency, --单位
       tmp_bond_structured.amt_cost        AS mon_bondamt, --月末债券余额（亿元）
       ...
       tmp_bond_structured.portfolio_nm, --组合名称
  ...
  FROM tmp_bond_structured
  LEFT JOIN vw_bond_issuer vw_issuer
    ON (tmp_bond_structured.secinner_id = vw_issuer.secinner_id)
  LEFT JOIN tmp_bond_pool
    ON (tmp_bond_structured.secinner_id = tmp_bond_pool.secinner_id)
  LEFT JOIN bond_basicinfo basinfo
    ON (tmp_bond_structured.secinner_id = basinfo.secinner_id)
  LEFT JOIN tmp_bond_party
    ON (tmp_bond_structured.secinner_id = tmp_bond_party.secinner_id)
  ...
  LEFT JOIN tmp_bond_creditchg
    ON (tmp_bond_structured.secinner_id = tmp_bond_creditchg.secinner_id)
  ...
  LEFT JOIN tmp_bond_warrantor
    ON (tmp_bond_structured.secinner_id = tmp_bond_warrantor.secinner_id)
;
